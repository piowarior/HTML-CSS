"""
AES-128 (ECB) - educational implementation with round visualization
This version prints the internal 4x4 state matrix in hexadecimal after each round.
"""

PLAINTEXT = "KOMPARASI ALGORITAM SIMETRIS AES DAN DES"
KEY = "CR001RABCIE512"

# --- AES tables ---
sbox = [

]

Rcon = [0x00,0x01,0x02,0x04,0x08,0x10,0x20,0x40,0x80,0x1B,0x36]


def xtime(a):
    return ((a << 1) ^ 0x1b) & 0xff if (a & 0x80) else (a << 1) & 0xff

def sub_word(word):
    return [sbox[b] for b in word]

def rot_word(word):
    return word[1:] + word[:1]

def key_expansion(key_bytes):
    Nk=4
    Nb=4
    Nr=10
    w=[0]*(Nb*(Nr+1)*4)
    for i in range(16):
        w[i]=key_bytes[i]
    for i in range(4,Nb*(Nr+1)):
        temp=[w[4*(i-1)+j] for j in range(4)]
        if i%Nk==0:
            temp=sub_word(rot_word(temp))
            temp[0]^=Rcon[i//Nk]
        for j in range(4):
            w[4*i+j]=w[4*(i-Nk)+j]^temp[j]
    round_keys=[]
    for r in range(Nr+1):
        round_keys.append(bytes(w[16*r:16*(r+1)]))
    return round_keys

def add_round_key(state,rk):
    return [s^k for s,k in zip(state,rk)]

def sub_bytes(state):
    return [sbox[b] for b in state]

def shift_rows(state):
    s=state[:]
    return [s[0],s[5],s[10],s[15],s[4],s[9],s[14],s[3],s[8],s[13],s[2],s[7],s[12],s[1],s[6],s[11]]

def mix_single_column(a):
    t=a[0]^a[1]^a[2]^a[3]
    u=a[0]
    a0=a[0]^t^xtime(a[0]^a[1])
    a1=a[1]^t^xtime(a[1]^a[2])
    a2=a[2]^t^xtime(a[2]^a[3])
    a3=a[3]^t^xtime(a[3]^u)
    return [a0&0xff,a1&0xff,a2&0xff,a3&0xff]

def mix_columns(state):
    out=[0]*16
    for c in range(4):
        col=[state[c+4*r] for r in range(4)]
        mixed=mix_single_column(col)
        for r in range(4):
            out[c+4*r]=mixed[r]
    return out

def print_matrix(state,label):
    print(label)
    for r in range(4):
        print([f"{state[c+4*r]:02X}" for c in range(4)])
    print()

def encrypt_block(block,round_keys):
    state=list(block)
    state=add_round_key(state,round_keys[0])
    print_matrix(state,"Round 0 (after AddRoundKey)")
    for r in range(1,10):
        state=sub_bytes(state)
        state=shift_rows(state)
        state=mix_columns(state)
        state=add_round_key(state,round_keys[r])
        print_matrix(state,f"Round {r}")
    state=sub_bytes(state)
    state=shift_rows(state)
    state=add_round_key(state,round_keys[10])
    print_matrix(state,"Round 10 (final)")
    return bytes(state)

def pkcs7_pad(data_bytes,block_size=16):
    pad_len=block_size-(len(data_bytes)%block_size)
    return data_bytes+bytes([pad_len])*pad_len

def ecb_encrypt(data_bytes,key_bytes):
    round_keys=key_expansion(key_bytes)
    cipher=b""
    for i in range(0,len(data_bytes),16):
        block=data_bytes[i:i+16]
        cipher+=encrypt_block(block,round_keys)
    return cipher

if __name__=='__main__':
    key_bytes=KEY.encode('utf-8')
    if len(key_bytes)<16:
        key_bytes+=b'\x00'*(16-len(key_bytes))
    else:
        key_bytes=key_bytes[:16]
    data_bytes=PLAINTEXT.encode('utf-8')
    padded=pkcs7_pad(data_bytes,16)
    print("PLAINTEXT:",PLAINTEXT)
    print("KEY (hex):",key_bytes.hex())
    cipher_bytes=ecb_encrypt(padded,key_bytes)
    print("Ciphertext (hex):",cipher_bytes.hex())